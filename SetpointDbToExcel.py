...
def monthly_report_for_client(snapshot, company, month, year, request):
...
def monthly_work_to_excel(snapshot, employee, month, year):
    workbook = xlsxwriter.Workbook(f'{employee}_{month}_{year}.xlsx')
    worksheet = workbook.add_worksheet()
...
def main():
    ...
    if not exists_in_DB(request, client):
    ...
    # dummy_values = [1, "אשדות יעקב מאוחד", 10, 2020]
    # dummy_values2 = [0, "אריה", 10, 2020]
    # snapshot = read_data(append_prefix(dummy_values2[0], dummy_values2[1]), dummy_values2[2], dummy_values2[3])
    # print(snapshot)
    # monthly_report_for_client(snapshot, dummy_values2[1], dummy_values2[2], dummy_values2[3], dummy_values2[0])
...
def append_prefix(request, client):
    ...
    if request == 3:
        return "Companies/" + client
    else:
        return "Others/" + client


def exists_in_DB(request, client):
    if request == 0:
        return client in employeesToHebrew.values()
    if request == 1:
        return client in kibbutzimToEnglish
    else:
        return False
